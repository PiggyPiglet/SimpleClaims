...
public class GlobalBreakBlockEventSystem extends WorldEventSystem<EntityStore, BreakBlockEvent> {
    ...
    public void handle(@Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull BreakBlockEvent event) {
        println("GlobalBreakBlockEventSystem: handling global break block event at " + event.getTargetBlock());
        ...
        if (playerUUID == null) {
            ...
            println("GlobalBreakBlockEventSystem: no nearby projectile found, denying in all claimed chunks");
            ...
            if (!ClaimManager.getInstance().isAllowedToInteract(null, worldName, x, z, PartyInfo::isBlockBreakEnabled)) {
                println("GlobalBreakBlockEventSystem: block is in claimed chunk, cancelling");
                event.setCancelled(true);
            }
            return;
        }

        println("GlobalBreakBlockEventSystem: found projectile owner: " + playerUUID);
        ...
        if (!ClaimManager.getInstance().isAllowedToInteract(playerUUID, worldName, x, z, PartyInfo::isBlockBreakEnabled)) {
            println("GlobalBreakBlockEventSystem: player " + playerUUID + " is not allowed to break blocks here, cancelling");
            event.setCancelled(true);
        } else {
            println("GlobalBreakBlockEventSystem: player " + playerUUID + " is allowed to break blocks here");
        }
    }
